function out=Similarity_matrix_via_DMC_symmetric(STRM,sigma,K,szsub)
%Input: STRM=cell array with each cell holding a dat-like matrix (the data
%manifold)
%See DMC_symmetric for info on inputs
...
%     j
    for k=1:j
        %pdist2(Phop{k},Phop{j},'euclidean')'=[Mk x Mj] distance matrix
            out(j,k)=DMC_symmetric(STRM{j,1},STRM{k,1},sigma,K);
    end
end
out=symmetrize(out);
